...
def detectionScoreCalculation(detection):
    ...
    detection_class = []
    for cls in detect_class[:10]:
        if cls == 1:
            detection_class.append('Income Tax Hindi-logo')
        elif cls == 2:
            detection_class.append('Income Tax English-logo')
        elif cls == 3:
            detection_class.append('Amblem-logo')
        elif cls == 4:
            detection_class.append('Government Of India Hindi-logo')      
        elif cls == 5:
            detection_class.append('Government Of India English-logo')
        elif cls == 6:
            detection_class.append('Permanent Account Number')
        elif cls == 7:
            detection_class.append('Signature')
        elif cls == 8:
            detection_class.append('Government Of India-Hologram')   
        elif cls == 9:
            detection_class.append('Pancard')
        elif cls == 10:
            detection_class.append('QR Code')
                              

    detection_score = []   
    for score in detect_score[:10]:
        detection_score.append(score*100)
    
    calculated_score = dict(zip(detection_class, detection_score))
    return calculated_score

def upload_image_pan(filestream):
    image = filestream
    ...
    detection_score_calculation = detectionScoreCalculation(detection=detections)
...
if __name__ == "__main__":
    image_path = "Test Cases/panVerifyTestCases/1.jpg"
    npimg = np.fromfile(image_path, np.uint8)
    image = cv2.imdecode(npimg, cv2.IMREAD_COLOR)
    res = upload_image_pan(image)
    print(res)
    print("Done!!!")