...
from aadharOCR import ocrAadhar
...
def upload_image_aadhar_doc(filestream):
    image = filestream
    ...
    is_aadhar = ocrAadhar(threshImageName)
...
if __name__ == "__main__":
    image_path = "test_images_aadhar/33.jpg"
    npimg = np.fromfile(image_path, np.uint8)
    image = cv2.imdecode(npimg, cv2.IMREAD_COLOR)
    res = upload_image_aadhar_doc(image)
    print(res)
    print("Done!!!")
